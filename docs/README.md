...
This package includes `avrt.dll`, `RS_ASIO.dll`, `RS_ASIO.ini`, `rocksmith-on-linux.sh`, `pipeasio-register`, `LICENSE`, and this README alongside `setup-guide.md`.

- Copy `avrt.dll`, `RS_ASIO.dll`, `RS_ASIO.ini` and `rocksmith-on-linux.sh` to the game folder.
  - The `RS_ASIO.ini` file is pre-configured to be used with the recommended environment. You may alter it as needed.

### Configuration

Check [setup-guide.md](setup-guide.md), included in this package.
...
- Use at your own risk. See the included `LICENSE` file for full license terms.
...
This package only includes what's needed to install and configure the patch. For the full technical details document, development conversation logs, and the latest version of this project, see the [GitHub repository](https://github.com/ferabreu/rocksmith-on-linux).
